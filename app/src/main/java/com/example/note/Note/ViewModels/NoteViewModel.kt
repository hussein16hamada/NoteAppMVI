...
import com.example.note.Note.offlineDb.NoteDataBase
...
class NoteViewModel : ViewModel() {
    ...
    val registerResponse: MutableLiveData<RegisterLoginResponse> = MutableLiveData()
...
//                    is NoteIntent.getNotesFromApi -> getNotesFromApi()
    ...
    private fun offlineNotesFromDb() {
        viewModelScope.launch {
            ...
            offlineNoteViewState.value = try {
                val res = NoteDataBase.getInstance(MyApplication.appContext).noteDao().getOfflineNotesMvi(false)
    ...
    private fun getNotesFromDataBase() {
        viewModelScope.launch {
            ...
            dbViewState.value = try {
                val res = NoteDataBase.getInstance(MyApplication.appContext).noteDao().allNotesMvi()
                NoteViewState.NoteListSuccess(res)
    ...
    fun addNoteTODataBase(noteResponseItem: NoteResponseItem) {
        ...
        NoteDataBase.getInstance(MyApplication.appContext)
            .noteDao()
...
//                        .show()
    ...
    fun deleteAllNotes() {
        NoteDataBase.getInstance(MyApplication.appContext).noteDao().deleteAllNotes()
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe(object : CompletableObserver {
                override fun onSubscribe(d: Disposable) {
    ...
    fun deleteFromDataBaseWithId(id: Int) {
        ...
        val single = NoteDataBase.getInstance(MyApplication.appContext)
            .noteDao()
            .deleteNote(id)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
